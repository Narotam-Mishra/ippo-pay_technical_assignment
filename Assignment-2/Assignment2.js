...
let minimumDifference = function(nums) {
    ...
    // calculate first helf and second half sum
    let firstHalfSum = firstHalf.reduce((a, c) => a + c);
    let secondHalfSum = secondHalf.reduce((a, c) => a + c);
    
    // find all combinations of sums of subsets made using of k integers
    ...
    for (let i = 1; i < firstDP.length; i++) {
        for (let num1 of firstDP[i]) {
            ...
            // i + remainingSubsetLen must equal length n 
            // as our goal is to create two arrays of length n
...
let arr = [2,-1,0,4,-2,-9];
let minAbsDiff = minimumDifference(arr);
